...
<div class="wrap">
    <div class="container content_wrap">
        <form  action="" method="" onSubmit="return false">
            <div class="row">
                <div class="col-12">
                    <h3 class="h3">일반 회원 목록</h3>
	                <button type="button" class="btn btn-light btn-sm position-absolute" onclick="location.href='./memberForm'">+ 신규 등록</button>  
                </div>                
								...
								<c:forEach items="${members}" var="member">
									...
									<tr>
										...
										<c:choose>
											...
											<c:otherwise>
											<td><button class="btn btn-sm btn-outline-light" onclick="location.href='./memberDetail?idx=${member.idx}';">상세</button></td>
											</c:otherwise>
										</c:choose>
									</tr>
...
<script>
	...
	//전체 선택
	var pageSelected = Number('${selectedCnt}');
	...
	function excel_download(){
		if(pageSelected == 0){open_modal_alert("다운로드할 회원을 선택해주세요."); return;}
	...
	$(document).ready(function() {
        ...
        $('#checkboxMemberAll').on('change', function() {
            if ($(this).is(':checked')) {
                var confirmResult = confirm("일반 회원 전체를 선택할까요?");
                if (!confirmResult) {
                    $(this).prop('checked', false);
                }else{
                	selectAllMember("selectAll1");
                	$('.checkboxMember').prop("checked", true);
                	$('#checkboxMemberPage').prop("checked", true);
                }
            } else {
                var confirmResult = confirm("일반 회원 전체를 선택 해제 할까요?");
                if (!confirmResult) {
                    $(this).prop('checked', true);
                }else{
                	selectAllMember("unselectAll1");
