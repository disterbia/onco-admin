	...
	var parameter = document.querySelector("meta[name='_csrf_parameter']").getAttribute("content");
	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src",`${CDN_HTTP}/editor/img/photoQuickPopup/btn_confirm2.png`);
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
	    	 ...
	    	 welBtnConfirm.attr("src",`${CDN_HTTP}/editor/img/photoQuickPopup/btn_confirm.png`);
    	 }
     }   
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
	        ...
	        aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button"' + `src=${CDN_HTTP}/editor/img/photoQuickPopup/btn_del.png  width="14" height="13" alt="첨부 사진 삭제"></a>`);
			aFileList.push('	</li> ');   
...
//    	sUploadURL= '/nse/nse_files/quick_photo_uploader/popup/FileUploader_html5.php'; 	//upload URL
    	...
    	sUploadURL= '/mng/upload'; 	//upload URL
    ...
    async function callAjaxForHTML5 (tempFile, sUploadURL){
...
//		oAjax.request(tempFile);
		...
		//JAVA 커스터마이징
		let res = await awaitPostMultipart(sUploadURL, {file: tempFile}, true);
		makeArrayFromString(res);
    }
